...
public class InitialFlowWriter implements DataTreeChangeListener<Node> {
    ...
    private final AtomicLong flowCookieInc = new AtomicLong(0x1a00000000000000L);
    ...
    private class InitialFlowWriterProcessor implements Runnable {
        ...
        void addInitialFlows(InstanceIdentifier<Node> nodeId) {
            ...
            //add send ip to controller flow
            writeFlowToController(nodeId, tableId, flowId, createSendToControllerFlow(flowTableId,KnownEtherType.Ipv4, flowPriority+10));

            //add send arp to controller flow
            writeFlowToController(nodeId, tableId, flowId, createSendToControllerFlow(flowTableId,KnownEtherType.Arp, flowPriority+10));

            //add send arp to controller flow
            writeFlowToController(nodeId, tableId, flowId, createSendToControllerFlow(flowTableId,KnownEtherType.Lldp, flowPriority+70));
        ...
        private Flow createSendToControllerFlow(Short tableId, KnownEtherType type, int priority) {

            // start building flow,
            FlowBuilder sendToController = new FlowBuilder()
                    .setTableId(tableId)
                    .setFlowName("send2controller");

            // use its own hash code for id.
            sendToController.setId(new FlowId(Long.toString(sendToController.hashCode())));

            EthernetMatchBuilder ethernetMatchBuilder = new EthernetMatchBuilder()
                    .setEthernetType(new EthernetTypeBuilder()
                            .setType(new EtherType(Long.valueOf(type.getIntValue()))).build());

            Match match = new MatchBuilder()
                    .setEthernetMatch(ethernetMatchBuilder.build())
                    .build();

            Action sendToControllerAction = new ActionBuilder()
                                                ...
                                                .build();
            // Create an Apply Action
            ApplyActions applyActions = new ApplyActionsBuilder().setAction(ImmutableList.of(sendToControllerAction))
                    .build();
            ...
            // Put our Instruction in a list of Instructions
            sendToController.setMatch(match)
                    .setInstructions(new InstructionsBuilder()
                            .setInstruction(ImmutableList.of(applyActionsInstruction))
                            .build())
                    .setPriority(priority)
